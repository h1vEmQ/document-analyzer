...
class OllamaService:
    ...
    def _create_comparison_prompt(self, doc1_content: str, doc2_content: str, 
        ...
        """
        prompt = f"""Ты эксперт по анализу документов. Сравни два документа и найди различия, сходства и изменения.
...
Пожалуйста, проведи детальное сравнение и предоставь результат в следующем JSON формате:
...
Анализируй документы внимательно и предоставь структурированный результат."""
    ...
    def analyze_document_sentiment(self, content: str) -> Dict[str, Any]:
        ...
        """
        prompt = f"""Проанализируй тональность и эмоциональную окраску следующего текста:

{content[:2000]}

Предоставь результат в JSON формате:
{{
    ...
    def extract_key_points(self, content: str) -> Dict[str, Any]:
        ...
        """
        prompt = f"""Извлеки ключевые моменты из следующего документа:
